...
``GEMINI_API_KEY`` (Task 15) -- the full pipeline this task orchestrates
touches both. Neither is available in this environment/session, so this
module is skipped here with an explicit reason, not a fabricated pass.
Written to run for real inside the Task 14 Linux/Docker environment
...
pytestmark = pytest.mark.skipif(
    not _CHROMADB_AVAILABLE or not _settings_for_skip_check.gemini_api_key.get_secret_value(),
    reason=(
        "Requires both chromadb (not installed in the native Windows venv, "
        "ADR-004) and a real GEMINI_API_KEY (Task 15) -- neither is "
        "available in this environment. Run inside the Task 14 Linux/Docker "
        "environment with GEMINI_API_KEY set to verify against live "
        "infrastructure."
    ),
)
...
async def test_modifying_one_file_updates_only_that_files_vectors_live(
    ...
    a_before = await chroma.query_similarity(
        repository_id, [0.0] * 768, top_k=10, metadata_filter={"file_path": "a.py"}
    )
    b_before = await chroma.query_similarity(
        repository_id, [0.0] * 768, top_k=10, metadata_filter={"file_path": "b.py"}
    )
